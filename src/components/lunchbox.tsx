...
export const LunchboxItem = ({ item }: props) => {
   ...
   return (
      <div key={item.id} className="lunchboxItem relative group flex flex-col justify-center items-center bg-white p-4 rounded-xl cursor-pointer shadow-lg shadow-zinc-400 border hover:border-zinc-400 transition">
         {item.promotion &&
         ...
         <div className="relative w-28 md:w-40 h-28 md:h-40 overflow-visible">
            <Image src={`/lunchbox/${item.image}`} alt={item.name}
               className="spinAnimation absolute"
